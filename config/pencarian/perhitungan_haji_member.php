...
function getJenis(){
	include '../koneksi.php';
	$query  = "SELECT paket_data_id, jenispaket FROM tbl_data_paket INNER JOIN tbl_jenis_paket USING(jenispaket_id)  Where jenispaket_id='H1' GROUP BY paket_data_id" ;
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_assoc($result)) {
...
function getTestimoni($kd_testi){
	include '../koneksi.php';
	$query =mysqli_query($koneksi, "SELECT DISTINCT travel_id, paket_data_id, tbl_testimoni.testimoni_id, id_member, nilai, COUNT(nilai) as byk_data, sum(nilai) as jmlh_nilai FROM tbl_data_paket LEFT JOIN tbl_testimoni USING(travel_id)  Where jenispaket_id='H1' GROUP BY paket_data_id");
	while ($row=mysqli_fetch_assoc($query)) {
		$data[]=$row;
	}
...
function getHarga($kd_harga){
	include '../koneksi.php';
	$query=mysqli_query($koneksi,"SELECT harga_paket from tbl_data_paket  Where jenispaket_id='H1' GROUP by paket_data_id");
	while ($row=mysqli_fetch_assoc($query)) {
...
for($i=0; $i<$n; $i++){
    $_SESSION['data'][$i]=[
        ...
        'harga' =>getHarga($i),
        'paket' =>(getDataHari($i)*$subkriteria_bobot['Durasi_perjalanan'])+(getKelasHotel($i)*$subkriteria_bobot['Kelas Hotel'])+(getRutePenerbangan($i)*$subkriteria_bobot['Rute Penerbangan']),
...
header('location:/spkumrohhajidela/index.php?url=Halaman_Pencarian_Haji_member');
}
